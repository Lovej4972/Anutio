...
const Input = forwardRef((props, ref) => {
  ...
  const labelStyle = {
    ...
    color: _animatedIsFocused.interpolate({
      inputRange: [0, 1],
      outputRange: [colors.textPrimary, colors.textPrimary],
    }),
  };
  return (
    <TouchableWithoutFeedback onPress={focusInput}>
      <View
        style={{
          marginVertical: props.noBorder?0: scaleVertical(10),
          // flex: 1,
        ...
        <View
          style={{
            ...
            minHeight: props.noBorder?0: scale(45),
            flexDirection: 'row',
            justifyContent: 'space-between',
            borderColor: props.noBorder?"transparent": props.error
              ? colors.error
              : state.isFocused
              ? colors.success
              : colors.primaryTint,
          ...
          {icon ? (
            <TouchableOpacity
              ...
              <IconGenerator tagName={icon} color={state.isFocused ? colors.success : colors.gray5} />
            </TouchableOpacity>
          ) : null}
        </View>
        <Text
